function Create(self)
	...
	for i=1, 3 do
		...
		fx1.Pos = self.Pos;
		fx1.Vel = (self.Vel * math.random()) + Vector(math.random()*2,0):RadRotate(math.random()*(math.pi*2));
		MovableMan:AddParticle(fx1);
	
		fx2 = CreateMOPixel("XM Trail Glow " .. math.random(5), "Psyclones.rte");
		fx2.Pos = self.Pos;
		fx2.Vel = (self.Vel * math.random()) + Vector(math.random()*7,0):RadRotate(math.random()*(math.pi*2));
